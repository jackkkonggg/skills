...
3. Before designing, inventory behavior and present the user with specific
   features, compatibility paths, edge cases, or guarantees that could be
   removed. Ask which may be descoped. If none are credible, ask whether strict
   parity is required. Do not infer permission from missing tests or low usage.
...
6. Finish with direct behavior evidence, relevant performance evidence, and a
